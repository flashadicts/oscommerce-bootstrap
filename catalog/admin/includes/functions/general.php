<?php
/*
  $Id: general.php 205 2011-03-21 11:43:13Z Rob $
...
// Text Mode
    ...
    function tep_address_format($address_format_id, $address, $html, $boln = '', $eoln = '', $ajax = '') {
    $address_format_query = tep_db_query("select address_format as format from " . TABLE_ADDRESS_FORMAT . " where address_format_id = '" . (int)$address_format_id . "'");
    $address_format = tep_db_fetch_array($address_format_query);

    $company = ((isset($address['company']) && $address['company'] != '') ? (($ajax != '') ? '<a id="company" data-table="orders" data-field="'.$ajax.'company" href="#" class="ajaxLink">' : '') . tep_output_string_protected($address['company']) . (($ajax != '') ? '</a>' : '') : '');
    if (isset($address['firstname']) && tep_not_null($address['firstname'])) {
      $firstname = tep_output_string_protected($address['firstname']);
      $lastname = tep_output_string_protected($address['lastname']);
    } elseif (isset($address['name']) && tep_not_null($address['name'])) {
      $firstname = (($ajax != '') ? '<a id="delivery_name" data-table="orders" data-field="'.$ajax.'name" href="#" class="ajaxLink">' : '') . tep_output_string_protected($address['name']) . (($ajax != '') ? '</a>' : '');
      $lastname = '';
    } else {
      $firstname = '';
      $lastname = '';
    }
    $street = (($ajax != '') ? '<a id="street_address" data-table="orders" data-field="'.$ajax.'street_address" href="#" class="ajaxLink">' : '') . tep_output_string_protected($address['street_address']) . (($ajax != '') ? '</a>' : '');
    ((isset($address['suburb']) && $address['suburb'] != '') ? $suburb = (($ajax != '') ? '<a id="suburb" data-table="orders" data-field="'.$ajax.'suburb" href="#" class="ajaxLink">' : '') . (isset($address['suburb']) ? tep_output_string_protected($address['suburb']) : '') . (($ajax != '') ? '</a>' : '') : '');
    $city = (($ajax != '') ? '<a id="city" data-table="orders" data-field="'.$ajax.'city" href="#" class="ajaxLink">' : '') . (isset($address['city']) ? tep_output_string_protected($address['city']) : '') . (($ajax != '') ? '</a>' : '');
    $state = (($ajax != '') ? '<a id="state" data-table="orders" data-field="'.$ajax.'state" href="#" class="ajaxLink">' : '') . (isset($address['state']) ? tep_output_string_protected($address['state']) : '') . (($ajax != '') ? '</a>' : '');
    if (isset($address['country_id']) && tep_not_null($address['country_id'])) {
      $country = tep_get_country_name($address['country_id']);
      ...
      $country = (($ajax != '') ? '<a id="country" data-table="orders" data-field="'.$ajax.'country" href="#" class="ajaxLink">' : '') . tep_output_string_protected($address['country']) . (($ajax != '') ? '</a>' : '');
    } else {
      $country = '';
    }
    $postcode = (($ajax != '') ? '<a id="postcode" data-table="orders" data-field="'.$ajax.'postcode" href="#" class="ajaxLink">' : '') . tep_output_string_protected($address['postcode']) . (($ajax != '') ? '</a>' : '');
    $zip = $postcode;
...
// Text Mode
    ...
    $statecomma = '';
    $streets = $street;
    if (isset($suburb) && $suburb != '') $streets = $street . $cr . $suburb;
    if ($country == '') $country = tep_output_string_protected($address['country']);
    if (isset($address['state']) && $address['state'] != '') $statecomma = $state . ', ';
  ...
  function tep_get_products_specification($product_id) {
    $product_query = tep_db_query("select products_specification from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "'");
    $product = tep_db_fetch_array($product_query);
