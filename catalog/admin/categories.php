<?php
/*
  $Id: categories.php 187 2010-12-01 11:12:10Z Rob $
      ...
      case 'update_product':
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          ...
          $sql_data_array = array('products_name' => tep_db_prepare_input($HTTP_POST_VARS['products_name'][$language_id]),
                                  ...
                                  'products_specification' => tep_db_prepare_input($HTTP_POST_VARS['products_specification']),
      ...
      case 'copy_to_confirm':
        if (isset($HTTP_POST_VARS['products_id']) && isset($HTTP_POST_VARS['categories_id'])) {
          ...
          if ($HTTP_POST_VARS['copy_as'] == 'link') {
            ...
            $description_query = tep_db_query("select language_id, products_name, products_description, products_specification, products_url from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$products_id . "'");
            while ($description = tep_db_fetch_array($description_query)) {
              tep_db_query("insert into " . TABLE_PRODUCTS_DESCRIPTION . " (products_id, language_id, products_name, products_description, products_specification, products_url, products_viewed) values ('" . (int)$dup_products_id . "', '" . (int)$description['language_id'] . "', '" . tep_db_input($description['products_name']) . "', '" . tep_db_input($description['products_description']) . "', '" . tep_db_input($description['products_specification']) . "', '" . tep_db_input($description['products_url']) . "', '0')");
            }
...
// check if the catalog image directory exists
  ...
  if (is_dir($root_images_dir)) {
    if (!tep_is_writable($root_images_dir)) $messageStack->add(ERROR_CATALOG_IMAGE_DIRECTORY_NOT_WRITEABLE, 'error');
    /* EOF  Select Product Image Directory */
...
// EOF Extra Fields Contribution
      $product_query = tep_db_query("select pd.products_name, pd.products_description, pd.products_specification, pd.products_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
      $product = tep_db_fetch_array($product_query);
...
?>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                    ...
                    <tr>
            <td class="main" valign="top"><?php echo TEXT_PRODUCTS_SPECIFICATION; ?></td>
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr> 
                <td class="main"><?php echo tep_draw_textarea_field('products_specification', 'soft', '70', '15',  tep_get_products_specification($pInfo->products_id)); ?></td>
              </tr>
            </table></td>
          </tr>
          <tr>
		  ...
		  <tr>
			<td class="main"><?php echo TEXT_PRODUCTS_IMAGE_DIRECTORY; ?></td>
           	<?php // place allowed sub-dirs in array, non-recursive
				$dir_array = array();
					/* Get subdirectories from image root */
					if ($handle = opendir($root_images_dir)) { 
                        ...
                        sort($dir_array);
               			...
               			die("<center><br><br><b> " . TEXT_PRODUCTS_DIRECTORY_DONT_EXIST_1 . " (" . $root_images_dir . ") " . TEXT_PRODUCTS_DIRECTORY_DONT_EXIST_2 . "</b></center><br><br>");
               		}
               		/* end Get subdirectories from image root */
...
<?php
    ...
    $product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_specification, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id  from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "'");
    $product = tep_db_fetch_array($product_query);
...
// EOF Extra Fields Contribution
          ...
          //echo "<br />" . $pInfo->products_specification;
         ?>
		</td>
