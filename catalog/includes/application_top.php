<?php
/*
  $Id: application_top.php 194 2010-12-21 07:53:38Z Rob $
...
// verify the IP address if the feature is enabled
  ...
  // BOF: [TiM's osC Solutions] Tax Switch
  if (!tep_session_is_registered('display_prices_with_tax')) {
    $display_prices_with_tax = DISPLAY_PRICE_WITH_TAX_DEFAULT;
    tep_session_register('display_prices_with_tax');
  }
  switch($display_prices_with_tax) {
    case 'true':
    ...
    case 'false':
      ...
      break;
    default:
      define('DISPLAY_PRICE_WITH_TAX', DISPLAY_PRICE_WITH_TAX_DEFAULT);
      break;
  }
// BOF: [TiM's osC Solutions] Tax Switch
...
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled
    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      case 'cust_order' :     if (tep_session_is_registered('customer_id') && isset($HTTP_GET_VARS['pid'])) {
                              ...
                              break;
                              // BOF: [TiM's osC Solutions] Tax Switch
      case 'toggle_tax':      if (isset($display_prices_with_tax)) {
                                $display_prices_with_tax = ($HTTP_GET_VARS['display_tax'] == 'true') ? 'true' : 'false';
                              }
                              tep_redirect(urldecode($HTTP_GET_VARS['uri']));
                              break;
    // EOF: [TiM's osC Solutions] Tax Switch
