    ...
    function execute() {
      ...
      if (isset($HTTP_GET_VARS['products_id'])) {
        ...
        if (tep_db_num_rows($manufacturer_query)) {
          ...
          $manufacturer_info_string = '<ul class="list-unstyled">';
          if (tep_not_null($manufacturer['manufacturers_image'])) $manufacturer_info_string .= '<li>' . tep_image(DIR_WS_IMAGES . $manufacturer['manufacturers_image'], $manufacturer['manufacturers_name']) . '</li>';
          if (tep_not_null($manufacturer['manufacturers_url'])) $manufacturer_info_string .= '<li><a href="' . tep_href_link(FILENAME_REDIRECT, 'action=manufacturer&manufacturers_id=' . $manufacturer['manufacturers_id']) . '" target="_blank">' . sprintf(MODULE_BOXES_MANUFACTURER_INFO_BOX_HOMEPAGE, $manufacturer['manufacturers_name']) . '</a></li>';
          $manufacturer_info_string .= '<li><a href="' . tep_href_link(FILENAME_DEFAULT, 'manufacturers_id=' . $manufacturer['manufacturers_id']) . '">' . MODULE_BOXES_MANUFACTURER_INFO_BOX_OTHER_PRODUCTS . '</a></li>';

          $data = '<div class="panel panel-default hidden-xs">' .
                  '<div class="panel-heading"><strong>' . MODULE_BOXES_MANUFACTURER_INFO_BOX_TITLE . '</strong></div>' .
                  '<div class="panel-body">' . $manufacturer_info_string .
                  '</div></div>';
